...
const TARGET_WORDS_PER_SLIDE = 24; // Строго 24 слова для одинаковой скорости начитки (в диапазоне 23-24)
const AI_RETRIES = 2;
    ...
    const response = await fetchWithTimeout('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: `Ты профессиональный редактор новостных тикеров для телевизионных новостей. Твоя задача - переписать исходный текст в РОВНО 3 новостных заголовка для бегущей строки.

КРИТИЧЕСКИ ВАЖНО - КОЛИЧЕСТВО СЛОВ:
- Каждый заголовок ДОЛЖЕН содержать РОВНО 24 слова (строго!)
...
СТИЛЬ И КАЧЕСТВО ПЕРЕПИСЫВАНИЯ:
1. Сохраняй ВСЕ ключевые факты: точные даты, места, числа, имена собственные
2. Используй профессиональную новостную лексику - конкретную и информативную
3. Каждый заголовок должен быть самодостаточным и понятным без контекста
4. Избегай повторов и лишних слов - каждое слово должно нести смысл
5. Структура: подлежащее + сказуемое + обстоятельства (кто, что сделал, где, когда)
6. Используй активный залог вместо пассивного ("пожарные спасли" вместо "были спасены")
7. НЕ добавляй информацию, которой нет в исходном тексте
8. НЕ добавляй эмоции, оценки, метафоры, восклицания - только факты
9. НЕ используй местоимения без ясного контекста - называй конкретно

ХРОНОЛОГИЯ:
- Слайд 1: начало событий, первая часть истории (24 слова)
- Слайд 2: развитие событий, средняя часть истории (24 слова)
- Слайд 3: завершение/итоги, финальная часть истории (24 слова)

ПРИМЕРЫ ХОРОШИХ ЗАГОЛОВКОВ:
- "Масштабные наводнения охватывают запад штата Вашингтон, уровень воды в реке Снохомиш достиг критической отметки в воскресенье."
- "Спасательные службы эвакуируют жителей из затопленных районов округа Снохомиш, закрыты основные автомагистрали и школы."
- "Метеорологи прогнозируют продолжение ливней в течение недели, уровень воды может подняться ещё на два метра к среде."

ФОРМАТ ОТВЕТА: Только JSON-массив из 3 строк, каждая строго 24 слова:
["первый заголовок ровно 24 слова", "второй заголовок ровно 24 слова", "третий заголовок ровно 24 слова"]`
          },
          {
            role: 'user',
            content: `Перепиши этот текст в 3 новостных заголовка. КАЖДЫЙ заголовок должен содержать РОВНО 24 слова (строго одинаковое количество для всех трёх):\n\n${text}`
          }
        ]
      })
    });
