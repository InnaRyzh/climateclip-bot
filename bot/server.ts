      ...
      if (fileId) {
        ...
        if (currentCount < requiredCount) {
          ...
          await bot.sendMessage(chatId, 
            ...
            'Пример: Индонезия, Бразилия, Япония, Саудовская Аравия / 12-13 декабря 2025\n\n' +
            '⚠️ Важно: дата должна быть в формате "12-13 декабря 2025" или "12 декабря 2025"'
          );
          } else {
             await bot.sendMessage(chatId,
              'Все видео получены! 📝\n\n' +
  ...
  if (state.step === 'waiting_info' && text) {
    if (state.template === 'grid') {
      ...
      if (countriesLine && dateLine) {
        // Валидация формата даты для Grid: "12-13 декабря 2025" или "12 декабря 2025"
        const datePattern = /^\d{1,2}(-\d{1,2})?\s+(января|февраля|марта|апреля|мая|июня|июля|августа|сентября|октября|ноября|декабря)\s+\d{4}$/i;
        
        if (!datePattern.test(dateLine)) {
          await bot.sendMessage(chatId, 
            '❌ Неверный формат даты!\n\n' +
            'Для шаблона "Сетка" дата должна быть в формате:\n' +
            '• 12-13 декабря 2025\n' +
            '• 12 декабря 2025\n\n' +
            'Пожалуйста, отправьте дату в правильном формате.'
          );
          return;
        }
  ...
  try {
    ...
    if (state.newsTickers && state.newsTickers.length > 0 && state.newsTickers.some(t => t && t.trim().length > 0)) {
      try {
        ...
        const CTA_TEXT = "О причинах учащения природных катастроф и прогнозах на ближайшие годы в новом видео Эгона Чолакяна на канале Время правды";
